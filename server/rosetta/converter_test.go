...
func (s *ConverterTestSuite) TestSigningComponents() {
	...
	s.Run("length signers data does not match signers", func() {
		_, _, err := s.c.ToRosetta().SigningComponents(s.unsignedTx, &rosetta.ConstructionMetadata{GasPrice: "10" + sdk.MustGetBaseDenom()}, nil)
		s.Require().ErrorIs(err, crgerrs.ErrBadArgument)
	})
	...
	s.Run("length pub keys does not match signers", func() {
		_, _, err := s.c.ToRosetta().SigningComponents(
			s.unsignedTx,
			&rosetta.ConstructionMetadata{GasPrice: "10" + sdk.MustGetBaseDenom(), SignersData: []*rosetta.SignerData{
				{
					AccountNumber: 0,
					Sequence:      0,
				},
