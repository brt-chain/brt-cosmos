...
var OneBaseDenomInWei sdk.Int = sdk.NewInt(1_000_000_000_000)
...
func (k BaseSendKeeper) SubWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if amt.LTE(currentWeiBalance) {
		// no need to change base denom balance
		return k.setWeiBalance(ctx, addr, currentWeiBalance.Sub(amt))
	}
	currentBaseDenomBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	currentAggregatedBalance := currentBaseDenomBalance.Mul(OneBaseDenomInWei).Add(currentWeiBalance)
	...
	baseDenomBalance, weiBalance := SplitBaseDenomWeiAmount(postAggregatedbalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), baseDenomBalance), true); err != nil {
		return err
	}
...
func (k BaseSendKeeper) AddWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if postWeiBalance.LT(OneBaseDenomInWei) {
		// no need to change base denom balance
		return k.setWeiBalance(ctx, addr, postWeiBalance)
	}
	currentBaseDenomBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	baseDenomCredit, weiBalance := SplitBaseDenomWeiAmount(postWeiBalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), currentBaseDenomBalance.Add(baseDenomCredit)), true); err != nil {
		return err
	}
	return k.setWeiBalance(ctx, addr, weiBalance)
}
...
func SplitBaseDenomWeiAmount(amt sdk.Int) (sdk.Int, sdk.Int) {
	return amt.Quo(OneBaseDenomInWei), amt.Mod(OneBaseDenomInWei)
}
