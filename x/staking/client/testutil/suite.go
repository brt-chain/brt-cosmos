...
func (s *IntegrationTestSuite) SetupSuite() {
    ...
    unbond, err := sdk.ParseCoinNormalized("10" + sdk.MustGetBaseDenom())
	s.Require().NoError(err)
...
func (s *IntegrationTestSuite) TestNewCreateValidatorCmd() {
	...
	testCases := []struct {
		...
		},
		{
			...
			[]string{
                fmt.Sprintf("--%s=%d%s", cli.FlagAmount, 100, sdk.MustGetBaseDenom()),
				fmt.Sprintf("--%s=AFAF00C4", cli.FlagIdentity),
				fmt.Sprintf("--%s=https://newvalidator.io", cli.FlagWebsite),
				fmt.Sprintf("--%s=contact@newvalidator.io", cli.FlagSecurityContact),
				fmt.Sprintf("--%s='Hey, I am a new validator. Please delegate!'", cli.FlagDetails),
			...
			if tc.expectErr {
				...
				for i := 0; i < len(events); i++ {
					if events[i].GetType() == "create_validator" {
                        ...
                        require.Equal(attributes[1].Value, "100"+sdk.MustGetBaseDenom())
						break
					}
				}
			}
...
unbonding_time: 1814400s`,
		},
		{
			"with json output",
			[]string{fmt.Sprintf("--%s=json", tmcli.OutputFlag)},
            fmt.Sprintf(`{"unbonding_time":"1814400s","max_validators":100,"max_entries":7,"historical_entries":10000,"bond_denom":"%s",min_commission_rate: "0.000000000000000000"}`, sdk.MustGetBaseDenom()),
		},
	}
	for _, tc := range testCases {
		tc := tc
