...
func (suite *SimTestSuite) testSimulateMsgWithdrawValidatorCommission(tokenName string) {
	...
	suite.Require().NoError(simapp.FundModuleAccount(suite.app.BankKeeper, suite.ctx, distrAcc.GetName(), sdk.NewCoins(
        ...
        sdk.NewCoin(sdk.MustGetBaseDenom(), sdk.NewInt(5)),
	)))
	...
	valCommission := sdk.NewDecCoins(
        ...
        sdk.NewDecCoinFromDec(sdk.MustGetBaseDenom(), sdk.NewDec(1).Quo(sdk.NewDec(1))),
	)
...
func (suite *SimTestSuite) TestSimulateMsgFundCommunityPool() {
	...
	suite.Require().True(operationMsg.OK)
    suite.Require().Equal("4896096"+sdk.MustGetBaseDenom(), msg.Amount.String())
