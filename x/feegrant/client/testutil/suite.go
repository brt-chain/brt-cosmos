...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	grant, err := feegrant.NewGrant(granter, grantee, &feegrant.BasicAllowance{
		SpendLimit: sdk.NewCoins(sdk.NewCoin(sdk.MustGetBaseDenom(), sdk.NewInt(100))),
	})
	s.Require().NoError(err)
...
func (s *IntegrationTestSuite) createGrant(granter, grantee sdk.Address) {
	...
	fee := sdk.NewCoin(sdk.MustGetBaseDenom(), sdk.NewInt(100))
	...
	}{
		{
			...
			append(
				[]string{
					"wrong_granter",
					"cosmos1nph3cfzk6trsmfxkeu943nvach5qw4vwstnvkl",
					fmt.Sprintf("--%s=%s", cli.FlagSpendLimit, "100"+sdk.MustGetBaseDenom()),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
				commonFlags...,
			),
		...
		},
		{
			...
			append(
				[]string{
					...
					"cosmos1nph3cfzk6trsmfxkeu943nvach5qw4vwstnvkl",
					fmt.Sprintf("--%s=%s", cli.FlagSpendLimit, "100"+sdk.MustGetBaseDenom()),
					fmt.Sprintf("--%s=%s", cli.FlagPeriodLimit, "10"+sdk.MustGetBaseDenom()),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
...
func (s *IntegrationTestSuite) TestFilteredFeeAllowance() {
	...
	spendLimit := sdk.NewCoin(sdk.MustGetBaseDenom(), sdk.NewInt(1000))
